...
- (void)loadView
{
    ...
    _vImageNameBG                              = [[UILabel alloc] initWithFrame:CGRectMake(10, 74, 95, 20)];
    _vImageNameBG.layer.cornerRadius           = 3.0f;
    _vImageNameBG.layer.masksToBounds          = YES;
    _lblImageName                              = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 95, 20)];
    _lblImageName.textColor                    = [UIColor whiteColor];
    _lblImageName.font                         = [UIFont systemFontOfSize:12];
    _lblImageName.textAlignment                = NSTextAlignmentCenter;
    _lblImageName.text                         = @"201505201010";
    
    _vImageNameBG.backgroundColor              = [UIColor darkGrayColor];
    [_vImageNameBG addSubview:_lblImageName];
    [_innerContainer addSubview:_vImageNameBG];
...
- (void)resizeImageViewsWithRect:(CGRect)rect
{
    ...
    for (i = 0; i < count; i++) {
        ...
        photoView.frame = CGRectMake(dx, -40, rect.size.width, rect.size.height );
        dx += rect.size.width;
    }
}
...
- (void)enterFullscreen
{
    if (!_isThumbViewShowing)
    {
        ...
        CGRect frameBG = _vImageNameBG.frame;
        frameBG.origin.y -= 44;
        _vImageNameBG.frame = frameBG;
...
- (void)updateScrollSize
{
    ...
    [_scroller setContentSize:CGSizeMake(contentWidth, _scroller.frame.size.height-64)];
}
...
- (void)updateTitle
{
    ...
    if([_photoSource numberOfPhotosForPhotoGallery:self] > 0 )
    {
        if([_photoSource respondsToSelector:@selector(photoGallery:nameForPhotoAtIndex:)])
        {
            NSString *imgName = [_photoSource photoGallery:self nameForPhotoAtIndex:_currentIndex];
            _lblImageName.text = imgName;
        }
    }
}
...
- (void)didReceiveMemoryWarning {
    ...
    DLog(@"[FGalleryViewController] didReceiveMemoryWarning! clearing out cached images...");
